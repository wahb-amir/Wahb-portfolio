...
import ProjectStatusBanner from "@/app/Component/projects/ProjectStatusBanner";
import ProjectActionButtons from "@/app/Component/projects/ProjectActionButtons";
...
function SectionBox({
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-gray-100/80 bg-white shadow-sm dark:border-slate-700/50 dark:bg-slate-800/50">
      ...
      />
      <div className="px-5 pb-5 pt-5 text-left">
...
function StatCard({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex flex-col gap-1 rounded-xl border border-cyan-100/80 bg-cyan-50/60 px-4 py-3 dark:border-cyan-800/40 dark:bg-cyan-950/30 text-left">
      ...
      <div className="relative w-full rounded-xl bg-gray-50/70 p-4 dark:bg-slate-900/40">
        ...
        <div className="hidden lg:grid grid-cols-4 gap-6 relative">
          {layers.map((layer, lIdx) => {
            ...
            return (
              <div key={layer.id} className="relative flex flex-col gap-4">
                {/* Header Track */}
                <div className="flex items-center gap-2 border-b border-gray-200/80 pb-2 dark:border-slate-800">
                  <span className="flex h-5 w-5 items-center justify-center rounded-md bg-white text-[10px] font-black shadow-2xs dark:bg-slate-800 dark:text-slate-400">
                    {lIdx + 1}
                  </span>
                ...
                <div className="flex flex-col gap-4">
                  {currentNodes.map((node) => {
                    ...
                    return (
                      <div
                        key={node.id}
                        className={`relative rounded-xl border bg-white/90 p-3.5 shadow-sm transition-all duration-300 hover:shadow-md dark:bg-slate-800/80 ${meta.border}`}
                      >
                            ...
                            {systemEdges.map((edge, eIdx) => {
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <div className="relative overflow-hidden rounded-md bg-gray-50 px-2 py-1 text-[10px] font-medium text-gray-600 transition-colors group-hover/edge:bg-cyan-50/60 dark:bg-slate-900 dark:text-slate-400 dark:group-hover/edge:bg-cyan-950/20">
                                    {edge.label}
                                  </div>
                                </div>
                              );
        ...
        {/* Mobile / Tablet Responsive Pipeline Stack Layout */}
          ...
          {design.nodes.map((node, nIdx) => {
            ...
            return (
              <div key={node.id} className="relative flex flex-col">
                      ...
                      {systemEdges.map((edge, eIdx) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <p className="text-[11px] font-medium text-gray-600 dark:text-slate-400 pl-1">
                              ↳ {edge.label}
                            </p>
                          </div>
                        );
                ...
                {/* Vertical Step Connection Indicator */}
                {nIdx < design.nodes.length - 1 && (
                  <div className="mx-6 h-4 w-0.5 bg-gradient-to-b from-gray-200 to-transparent dark:from-slate-700" />
                )}
              </div>
            );
          })}
...
export default async function ProjectPage({ params }: Props) {
  ...
  const status = project.status ?? "working"; // safer fallback if status is missing
    ...
    <>
      ...
      >
        <div className="mx-auto max-w-5xl px-4 pb-24 pt-8 sm:px-6 lg:px-8">
          ...
          <div className="fade-up fade-up-1 mb-8 text-left">
          ...
          {cs.tlDr && (
            <div className="fade-up fade-up-2 relative mb-8 overflow-hidden rounded-2xl border border-blue-100/80 bg-gradient-to-br from-blue-50 to-cyan-50/60 px-5 py-4 dark:border-blue-800/40 dark:from-blue-950/50 dark:to-cyan-950/30 text-left">
              <div
          ...
          {/* Integrated High-Fidelity Network Architecture Grid Diagram View */}
          {project.systemDesign?.nodes && (
            <div className="fade-up fade-up-3 mb-8">
              <SystemDesignVisualizer design={project.systemDesign} />
            </div>
          )}
          ...
          {images.length > 1 && (
            <div className="fade-up fade-up-4 mb-8">
              <p className="mb-3 text-[11px] font-bold uppercase tracking-widest text-gray-400 dark:text-slate-500 text-left">
                More Screenshots
              </p>
