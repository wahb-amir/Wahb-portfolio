...
export default function ProjectCardSSR({
  ...
  return (
    <>
      <style>{`
        @keyframes trophy-pop-card {
          from { opacity: 0; transform: scale(0.6) rotate(-10deg); }
          to   { opacity: 1; transform: scale(1) rotate(0deg); }
        }
        .badge-pop {
          animation: trophy-pop-card 0.4s cubic-bezier(0.22,1,0.36,1) 0.3s both;
        }
      `}</style>
      ...
      >
        ...
        <div className="relative w-full h-48 sm:h-56 overflow-hidden bg-gray-50 dark:bg-slate-900">
          ...
          {badge?.label && (
            <div className="absolute top-3 right-3 z-20 badge-pop">
              <div className="inline-flex flex-col items-end gap-0.5">
                <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wide bg-yellow-400 dark:bg-yellow-500 text-yellow-900 border border-yellow-300/80 dark:border-yellow-600/60 shadow-md shadow-yellow-400/30">
                  {badge.label}
                </span>
                {badge.sub && (
                  <span className="text-[9px] font-medium text-yellow-800 dark:text-yellow-300 bg-yellow-100/80 dark:bg-yellow-950/60 px-2 py-0.5 rounded-full border border-yellow-200/60 dark:border-yellow-800/40">
                    {badge.sub}
                  </span>
                )}
              </div>
        ...
        <div className="p-5 sm:p-6 flex flex-col flex-1 min-w-0 max-w-full overflow-hidden gap-4">
          ...
          {tech.length > 0 && (
            <div className="flex flex-wrap gap-1.5">
              ...
              {tech.length > 6 && (
                <span className="text-[11px] px-2.5 py-0.5 rounded-full font-medium bg-gray-100 dark:bg-slate-700/70 text-gray-400 dark:text-slate-500 border border-gray-200/60 dark:border-slate-600/40">
                  +{tech.length - 6}
                </span>
              )}
            </div>
          ...
          {/* TL;DR block */}
          {normalizedCS.tlDr && (
            <div className="relative overflow-hidden rounded-xl px-4 py-3.5 bg-gradient-to-br from-blue-50 to-cyan-50/60 dark:from-blue-950/50 dark:to-cyan-950/30 border border-blue-100/80 dark:border-blue-800/40">
              <span
                className={`absolute left-0 top-0 bottom-0 w-[3px] rounded-l-xl bg-gradient-to-b ${gradientClass}`}
                aria-hidden="true"
              />
              <p className="text-xs sm:text-[13px] leading-relaxed text-gray-700 dark:text-slate-300 line-clamp-3 pl-1.5">
                {normalizedCS.tlDr}
              </p>
            </div>
          )}
