...
function cleanObject<T extends Record<string, any>>(obj: T): any {
  ...
  if (Array.isArray(obj)) {
    const arr = obj
      .map((v) => cleanObject(v))
      .filter((v) => !(v == null || (typeof v === "string" && v.trim() === "") || (Array.isArray(v) && v.length === 0)));
  ...
  if (typeof obj === "object") {
    ...
    Object.entries(obj).forEach(([k, v]) => {
      ...
      if (cleaned !== undefined && !(typeof cleaned === "string" && cleaned.trim() === "") && !(Array.isArray(cleaned) && cleaned.length === 0)) {
        out[k] = cleaned;
      }
    });
...
function slugify(input?: string, fallback = "untitled") {
  ...
  return input.toString().trim().toLowerCase().replace(/\s+/g, "-").replace(/[^a-z0-9\-]/g, "").replace(/\-+/g, "-");
}
...
export default async function ProjectServer() {
  ...
  try {
    const { payload: svcPayload } = await getLatestProjectsPayload({ clientVersion: null });
    payload = { version: svcPayload.version ?? null, data: svcPayload.data ?? null };
  } catch (err) {
  ...
  const graphEntries = projects.map((p) => {
    ...
    const catSearch = (p.tags ?? []).join(" ") + (p.category ?? "");
    ...
    const languages = (Array.isArray(p.languages) && p.languages.length ? p.languages : undefined) ??
      (Array.isArray(p.techStack) ? p.techStack.filter((t) => /^(javascript|typescript|python|go|rust|java|c#|php|node)/i.test(t)) : undefined);

    const sameAs = [...(p.repo ? [p.repo] : p.repoUrl ? [p.repoUrl] : []), ...(p.liveLink ? [p.liveLink] : p.url ? [p.url] : [])].filter(Boolean);
    ...
    const softwareApp = cleanObject({
      ...
      aggregateRating: p.rating && typeof p.rating.value === "number" ? {
    ...
    const codeEntry = sameAs.length ? cleanObject({
  ...
  const graph = graphEntries.flatMap(e => [e.softwareApp, e.codeEntry]).filter(Boolean);
  ...
  const itemList = cleanObject({
    ...
    itemListElement: projects.map((p, i) => ({
      "@type": "ListItem",
      position: i + 1,
      item: { "@id": `${SITE_URL}/#project-${slugify(p.id ?? p.title ?? p.name)}` },
    })),
  });
  ...
  return (
    <section
      id="project-section"
      className="relative flex flex-col items-center py-12 px-4 sm:px-6 lg:px-8 text-center overflow-hidden z-10 bg-[#f9fafb] dark:bg-[#0f172a] bg-gradient-to-b from-[#00bfff44] to-[#00b1ff88] text-black dark:text-white"
      role="region"
      aria-labelledby="projects-heading"
      data-keywords="projects,portfolio,case-studies,projects-grid"
    >
      <h2 id="projects-heading" className="mb-4 font-extrabold tracking-tight text-[36px] sm:text-[44px]">
        My Projects
      </h2>
      ...
      <div id="projects-grid" role="list" aria-label="Projects grid" className="grid screen-min-850:grid-cols-2 screen-max-850:grid-cols-1 lg:grid-cols-2 gap-8 w-full max-w-6xl px-2 items-start">
        {projects.length === 0 ? [0, 1].map((i) => (
          ...
          <div key={p.id ?? p.title} role="listitem" aria-label={`Project: ${p.title ?? p.name}`} data-project-id={slugify(p.id ?? p.title)}>
