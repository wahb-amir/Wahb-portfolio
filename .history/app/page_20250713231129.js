...
import { loadCharShape } from "tsparticles-shape-char";

const particlesInit = async (engine) => {
  const { loadBasic } = await import("tsparticles-basic");
  const { loadCharShape } = await import("tsparticles-shape-char");

  await loadBasic(engine); // Base engine (motion, links, interactivity)
  await loadCharShape(engine); // Tech name particles
};
...
const particlesOptions = {
  ...
  interactivity: {
    events: {
      onHover: {
        enable: true,
        mode: "repulse", // push, repulse, grab
      },
    },
    modes: {
      repulse: {
