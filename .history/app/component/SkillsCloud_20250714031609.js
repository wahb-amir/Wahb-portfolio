...
// Big circle size & radius for more gap 🎯
const containerSize = 420;   // bigger container
const radius = 160;          // bigger radius for spacing logos further apart
...
export default function SkillsCloud() {
  ...
  return (
    <section
      id="skills"
      className="relative flex flex-col justify-start items-center min-h-screen px-4 xs:px-6 text-center bg-[#0a0f1a] text-white overflow-hidden pt-[env(safe-area-inset-top)]"
      style={{
        backgroundImage:
      ...
      >
        {skillsWithPosition.map(({ name, icon: Icon, x, y, color }, i) => (
          <motion.div
            ...
            className="absolute flex items-center justify-center cursor-pointer rounded-full shadow-lg w-[60px] h-[60px] bg-gray-900"
            style={{
              top: centerOffset,
              left: centerOffset,
              boxShadow:
