...
export default function SkillsCloud() {
  ...
  useEffect(() => {
    const handleResize = () => setMinHeight(getMinHeight());
    window.addEventListener("resize", handleResize);
    return () => window.removeEventListener("resize", handleResize);
  }, []);
  ...
  return (
    <section
      id="skills"
      className={`relative flex flex-col items-center justify-center px-4 sm:px-6 pt-[env(safe-area-inset-top)] pb-12 text-center overflow-hidden
      ...
      style={{
        ...
        backgroundImage: isDark
          ...
          : "radial-gradient(circle at top left, #7f5af0aa, transparent 70%), radial-gradient(circle at bottom right, #00dfd8aa, transparent 70%)",
      }}
    >
      <BackgroundEffect />
        ...
        >
          ...
          {learningPositions.map(({ name, icon: Icon, x, y, color }, i) => (
            <motion.div
              ...
              className="absolute flex flex-col items-center cursor-help rounded-full w-14 h-14"
              style={{
                top: centerOffset,
                left: centerOffset,
            ...
            >
              <motion.div
                className="flex items-center justify-center rounded-full w-10 h-10 mb-1 shadow-inner"
                style={{ color }}
                animate={{ boxShadow: `0 0 10px 2px ${color}` }}
              ...
              {/* Progress Bar */}
              <motion.div
                className={`w-10 h-1 rounded-full overflow-hidden bg-gray-300 dark:bg-gray-700`}
              >
                <motion.div
