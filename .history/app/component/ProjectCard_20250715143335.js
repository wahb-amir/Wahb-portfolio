...
const ProjectCard = ({
  ...
  <div className="mt-8 flex flex-col items-center bg-white rounded-lg shadow-md p-4 sm:p-6 w-full max-w-xs sm:max-w-md md:max-w-lg lg:max-w-xl xl:max-w-2xl mx-auto">
    ...
    <div className="flex flex-col sm:flex-row sm:space-x-4 space-y-2 sm:space-y-0 justify-center items-center w-full">
      <button
        onClick={() => window.open(liveLink, "_blank", "noopener,noreferrer")}
        className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 transition w-[250px] sm:w-auto text-center"
      >
        Live Demo
      </button>
      <button
        onClick={() => window.open(githubLink, "_blank", "noopener,noreferrer")}
        className="px-4 py-2 bg-gray-800 text-white rounded hover:bg-gray-900 transition w-[250px] sm:w-auto text-center"
      >
        GitHub
      </button>
    </div>
