      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <button
          ...
          className={`w-full p-3 rounded-xl transition-all duration-300 text-black font-bold
            ${status === 'sending' ? 'bg-yellow-300 animate-pulse' : 'bg-cyan-300 hover:bg-pink-300'}`}
        >
          {status === 'sending' ? 'Sending... 🚀' : status === 'sent' ? 'Sent! ✅' : 'Send it! 💌'}
        </button>
        {status === 'sent' && (
          <p className="text-center mt-3 text-green-400 animate-bounce">Gotcha! I’ll hit u back soon 👾</p>
        )}
      </form>
    </div>
  )
