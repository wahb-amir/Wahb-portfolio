...
// Skill data
const allSkills = [
  { name: "HTML", icon: SiHtml5, color: "#E34F26", type: "frontend", experience: "3 yrs" },
  { name: "CSS", icon: SiCss3, color: "#1572B6", type: "frontend", experience: "3 yrs" },
  { name: "Tailwind", icon: SiTailwindcss, color: "#38B2AC", type: "frontend", experience: "2 yrs" },
  { name: "JavaScript", icon: SiJavascript, color: "#F7DF1E", type: "frontend", experience: "3 yrs" },
  { name: "React", icon: SiReact, color: "#61DAFB", type: "frontend", experience: "2.5 yrs" },
  { name: "Next.js", icon: SiNextdotjs, color: "#000000", type: "frontend", experience: "2 yrs" },
  { name: "Node.js", icon: SiNodedotjs, color: "#339933", type: "backend", experience: "2.5 yrs" },
  { name: "Express", icon: SiExpress, color: "#000000", type: "backend", experience: "2 yrs" },
  { name: "MongoDB", icon: SiMongodb, color: "#47A248", type: "backend", experience: "2 yrs" },
  { name: "Framer Motion", icon: SiFramer, color: "#0055FF", type: "frontend", experience: "1 yr" },
];
    ...
    >
      ...
      <div className="z-20 flex gap-3 mt-2 mb-6">
        <button
          ...
          className={`px-4 py-1 rounded ${view === "cloud" ? "bg-cyan-500 text-black" : "bg-gray-800"}`}
        >
          Cloud
        </button>
        <button
          onClick={() => setView("grid")}
          className={`px-4 py-1 rounded ${view === "grid" ? "bg-cyan-500 text-black" : "bg-gray-800"}`}
        >
          Grid
        </button>
        <select
      ...
      {view === "cloud" ? (
        <div
          key={`cloud-${filter}`} // Forces rerender when toggling or changing filter
          ref={containerRef}
          className="relative z-20 mx-auto"
          style={{ width: containerSize, height: containerSize }}
        >
          {skillsWithPosition.map(
            ({ name, icon: Icon, x, y, color, experience }, i) => (
              <motion.div
                key={`${name}-${filter}`}
        ...
        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-6 z-20">
          {skills.map(({ name, icon: Icon, color, experience }) => (
            <div
              key={name}
            ...
            >
              ...
              <span className="text-xs text-cyan-400">{experience}</span>
            </div>
          ))}
        </div>
      )}
