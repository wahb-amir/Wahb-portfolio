...
export default function SkillsCloud() {
  ...
  // useInView with triggerOnce: false to re-trigger when scrolled in again
  ...
  // Filter skills based on filter state
  const skills = allSkills.filter((skill) =>
    filter === "all" ? true : skill.type === filter
  );

  // Calculate position for skill bubbles in circle layout
  ...
  // Use a key to force remount on filter or view change to re-run animations
  const animationKey = `${filter}-${view}-${isInView}`;
  ...
  return (
    ...
    >
      ...
      {view === "cloud" ? (
        <div
          key={animationKey} // forces re-mount to reset animation
          ref={containerRef}
