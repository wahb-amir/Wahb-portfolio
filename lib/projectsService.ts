 ...
 * Data is imported at build time from projects.data.json and bundled
 * into the server module. Zero network round-trips on page load.
 *
 * To restore Redis/DB later:
 *   1. Swap this file with projectsService.db.ts (keep this one as .static.ts)
 *   2. The ProjectsPayload type and getLatestProjectsPayload signature are
...
/**
 * Returns the full projects payload synchronously.
 * The async wrapper is kept so call sites are forward-compatible
 * with the DB version when you re-enable it.
 */
export async function getLatestProjectsPayload(options?: {
  clientVersion?: number | null;
}): Promise<{ payload: ProjectsPayload; fromCache: boolean }> {
  ...
  // Mimic the "client already up-to-date" short-circuit so call sites
  // behave consistently when the DB version is restored.
  if (clientVersion != null) {
    return {
      payload: { version: null, data: [] },
      fromCache: true,
    };
  }
...
/**
 * Convenience: get the raw array directly (for server components
...
/**
 * Convenience: get a single project by id.
 * Returns undefined if not found.
 */
export async function getProjectById(id: string): Promise<Project | undefined> {
  return (projects as Project[]).find((p) => p.id === id);
}